// Script to update model-loader.js with deduplicated vocabulary
const fs = require('fs');

// Read the original file
const content = fs.readFileSync('docs/model-loader.js', 'utf8');

// Read the deduplicated vocabulary
const deduplicatedVocab = fs.readFileSync('deduplicated_vocab_structure.js', 'utf8');
...
// Update decoder limits to match new vocabulary size (1992)
let updatedContent = newContent.replace(/for \(let j = 0; j < Math\.min\(lastPositionLogits\.length, \d+\); j\+\+\)/g, 
                                        'for (let j = 0; j < Math.min(lastPositionLogits.length, 1992); j++)');

// Save the updated file
fs.writeFileSync('docs/model-loader.js', updatedContent);
console.log('Successfully updated model-loader.js with deduplicated vocabulary (1992 unique tokens)');