// Script to update model-loader.js with expanded vocabulary v2
const fs = require('fs');
...
const expandedVocab = fs.readFileSync('expanded_vocab_structure_v2.js', 'utf8');
...
// Update decoder limits to match new vocabulary size (2327)
let updatedContent = newContent.replace(/for \(let j = 0; j < Math\.min\(lastPositionLogits\.length, \d+\); j\+\+\)/g, 
                                        'for (let j = 0; j < Math.min(lastPositionLogits.length, 2327); j++)');

// Save the updated file
fs.writeFileSync('docs/model-loader.js', updatedContent);
console.log('Successfully updated model-loader.js with expanded vocabulary (2327 unique tokens)');